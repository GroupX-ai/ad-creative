...
await Promise.all(BANNERS.flatMap(b => SHAPES.map(async ([shape, size]) => {
  try {
    ...
    let bytes = await save(url, file);
    if (bytes < 10000) throw new Error("short download " + bytes);
...
console.log("\ndone:", log.filter(x => !x.error).length, "of", BANNERS.length + VIDEOS.length);
